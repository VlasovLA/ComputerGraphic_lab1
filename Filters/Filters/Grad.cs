...
namespace Filters
{
    class Grad : Filters
    {
        ...
        public override Bitmap processImage(Bitmap sourceImage, BackgroundWorker worker)
        {
            ...
            {
                worker.ReportProgress((int)((float)(y - MH) / (sourceImage.Height + MH) * 40));
            ...
            for (int y = MH; y < sourceImage.Height - MH; y++)
            {
                worker.ReportProgress(40 + (int)((float)(y - MH) / (sourceImage.Height + MH) * 40));
                if (worker.CancellationPending)
                    return null;
                for (int x = MW; x < sourceImage.Width - MW; x++)
                {
            ...
            for (int x = 0; x < sourceImage.Width; x++)
            {
                ...
                {
                    Color srcColor = resultImage.GetPixel(x, y);
                    Color tmpColor = tmpImage.GetPixel(x, y);
                    Color resColor = Color.FromArgb(Clamp(srcColor.R - tmpColor.R, 0, 255), Clamp(srcColor.G - tmpColor.G, 0, 255), Clamp(srcColor.B - tmpColor.B, 0, 255));
